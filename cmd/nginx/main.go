...
func runBrotliBuild(d Deps, force, dryRun bool) int {
	...
	if out, err := aptInstall(d, append([]string{"-y"}, brotliBuildDeps...)...); err != nil {
...
func runNginxUpgrade(d Deps, dryRun, force, noReload bool) int {
	...
	if out, err := aptInstall(d, "--only-upgrade", "-y", "nginx"); err != nil {
...
//
// PREV_PKG holds the BARE version string (e.g. "1.26.3-3+deb13u5") — the
// equals sign is hard-coded into the apt-get invocation below. Passing a
// pre-equalsed value would produce `nginx==1.2.3`, which apt parses as
// package name `nginx=` (split on first `=`) → "Unable to locate package".
...
func runInstall(d Deps, channelStr string, brotliMode BrotliMode, dryRun, userForce bool) int {
	...
	if out, err := aptInstall(d, "-y", "nginx"); err != nil {
...
func ensureNginxOrgRepo(d Deps, channel nginxChannel, force bool) error {
	...
	if out, err := aptInstall(d, "-y", "curl", "ca-certificates", "gnupg"); err != nil {
...
func ensureBrotli(d Deps) bool {
	...
	out, err := aptInstall(d, append([]string{"-y"}, brotliPackages...)...)
	if err != nil {
...
// aptInstall runs `apt-get install <args>` non-interactively. Sets
// DEBIAN_FRONTEND=noninteractive (suppresses debconf prompts) AND passes
// dpkg's --force-confdef/--force-confold (keeps the existing conffile on
// upgrade without prompting). Without the dpkg flags, dpkg blocks on a
// y/n prompt for any locally-modified /etc/* file and dies on EOF when
...
// prompt.
func aptInstall(d Deps, args ...string) ([]byte, error) {
	full := append([]string{
		"-o", "Dpkg::Options::=--force-confdef",
		"-o", "Dpkg::Options::=--force-confold",
		"install",
	}, args...)
	return d.Exec.RunEnv("apt-get", []string{"DEBIAN_FRONTEND=noninteractive"}, full...)
}
